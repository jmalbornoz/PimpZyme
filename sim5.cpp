...
main()
{
   float Vmax_dir = 8000.0, Vmax_rev = 0.0, S_05_dir = 25.0, S_05_rev = 20.0e6, Mu = 40000.0, S, P;
   float vel_dir, vel_rev, v_teor, v_hill, m_enz, a, b, c;
   int n_S, n_P, nP_prod, nS_prod;
   
   const int niter = 300000, NENZ = 50, Nsub = 4;
   ...
   for(int i = 0; i < NENZ; i++)
      enz[i].setMulti(Nsub, Vmax_dir, Vmax_rev, S_05_dir, S_05_rev, a, 0, Mu);
   ...
   float DeltaS = 5;	// incremento sutrato, micromolares
   ...
   while(S <= Smax){
         ...
         myfile << S/Ks << '\t' << vel_dir - vel_rev << '\t' << v_hill << '\t' << v_teor << endl;
