// MONO6.CPP
// Definicion de las funciones miembro de la clase Mono6
...
void Mono6::setenz(const float vmax_d, const float vmax_r, const float km_d, const float km_r, const float mu) 
{
...
void Mono6::advancePhase(int& nS, int& nP, int& nS_out, int& nP_out)
{ 
   // obtiene paso de la simulacion desde singleton instanciado en main()
   float step = Timestep::Instance().getStep();
   ...
   if(phase == 0){
      ...
      // la enzima no ha adquirido sustrato aun
      if(dir == 0){      
         if(z1 < p_d && z2 > p_r){
            dir = 1;
            p = p_d;
            ...
            return;    // la enzima no adquiere sustrato
      }
      else if(dir != 0){      // la direccion de la enzima ya ha sido determinada (oligomeros)
         if(z1 < p_d && z2 > p_r && dir == 1)
            p = p_d;
         else if(z1 > p_d && z2 < p_r && dir == -1)
            p = p_r;
         else
            return;    // la enzima no adquiere sustrato
      }
